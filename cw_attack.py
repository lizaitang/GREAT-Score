...
data=open("cw_result_final.txt",'a')
...
train_X1=train_X[0:1000]
train_y=train_y[0:1000]

target_classes=np.zeros(1000)
...
model_list=['Rebuffi2021Fixing_70_16_cutmix_extra','Gowal2020Uncovering_extra','Rebuffi2021Fixing_70_16_cutmix_ddpm','Rebuffi2021Fixing_28_10_cutmix_ddpm','Augustin2020Adversarial_34_10_extra']
...
#model_list=['Engstrom2019Robustness','Rice2020Overfitting','Rony2019Decoupling','Ding2020MMA']
...
for model_name in model_list:
    ...
    l2_seq=np.zeros(50,dtype=float)
    acc_seq=np.zeros(50,dtype=float)
    num=np.zeros(50,dtype=float)
    ...
    #print(math.sqrt(math.pi/2)*difference,file=data)
    #a=np.mean(difference[0:int(num_correct)])
    ...
    atk = fb.attacks.L2CarliniWagnerAttack(binary_search_steps=1, initial_const=1,
                                        confidence=0, steps=1000, stepsize=0.0005)
            
    start1 = datetime.datetime.now()
    for idx in range(50):
        
        start = datetime.datetime.now()
        criterion = fb.criteria.TargetedMisclassification(target_classes[20*idx:20*idx+20])
        images1=images[20*idx:20*idx+20]
        labels1=labels[20*idx:20*idx+20]
